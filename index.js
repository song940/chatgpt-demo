...
const App = () => {
  ...
  return [
    ...
    h('form', { className: "flex", onSubmit: handleSubmit }, [
      h('select', { className: "select", onChange: e => setRole(e.target.value) }, Object.entries(roles).map(([k, x]) => h('option', { value: k, selected: role === k ? 'selected' : '' }, x.name))),
      h('input', { value: prompts, className: "input", placeholder: "Enter something...", onInput: e => setPrompts(e.target.value) }),
